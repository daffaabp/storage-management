...
const OtpModal = ({ accountId, email }: { accountId: string; email: string }) => {
    ...
    const handleSubmit = async (e: React.MouseEvent<HTMLButtonElement>) => {
        ...
        try {
            ...
            console.log("Gagal untuk memverifikasi OTP", error);
        }
    ...
    const handleResendOtp = async () => {
        // memanggil API untuk mengirim OTP ulang
            ...
            <AlertDialogContent className='shad-alert-dialog'>
                <AlertDialogHeader className='relative flex justify-center'>
                    <AlertDialogTitle className='h2 text-center'>
                        Masukkan OTP Anda
                        <Image src="/icons/close-dark.svg" alt="close" width={20} height={20} className="otp-close-button" onClick={() => setIsOpen(false)} />
                    </AlertDialogTitle>
                    <AlertDialogDescription className='subtitle-2 text-center text-light-100'>
                        Kami telah mengirimkan kode ke <span className='pl-1 text-brand'>{email}</span>
